...
(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #${CACHE_PANEL_ID} .cgat-cache-controls .cgat-cache-actions {
        display: inline-flex;
        align-items: center;
        gap: 8px;
        flex-wrap: wrap;
      ...
      #${CACHE_PANEL_ID} .cgat-cache-controls .cgat-cache-meta {
      ...
      #${CACHE_PANEL_ID} .cgat-cache-controls button[disabled] {
        opacity: 0.45;
        cursor: not-allowed;
      }
  ...
  function looksLikeAssistantMeta(text) {
    ...
    if (/^thought for \d+s$/i.test(String(text || "").trim())) return true;
  ...
  function renderCachedMessages() {
    ...
    const shown = state.payloadLoadedOlderCount;
    const total = state.payloadOlderTotalCount;
    ...
    panel.innerHTML = `
      <div class="cgat-cache-controls">
        <span class="cgat-cache-actions">
          <button type="button" class="cgat-cache-load-btn"${remaining <= 0 ? " disabled" : ""}>↑ Load previous ${step}</button>
          <button type="button" class="cgat-cache-show-all-btn"${remaining <= 0 ? " disabled" : ""}>Show all</button>
        </span>
        <span class="cgat-cache-meta">Real cached turns shown: ${shown}/${total}${remaining > 0 ? ` · ${remaining} remaining` : ""}</span>
      </div>
      ${activityHtml}
    ...
    panel.querySelector(".cgat-cache-load-btn")?.addEventListener("click", () => {
      requestCachedMessages("chunk");
    });
    panel.querySelector(".cgat-cache-show-all-btn")?.addEventListener("click", () => {
      requestCachedMessages("all");
    });
  ...
  function updateBar(hiddenCount, settings) {
    ...
    loadBtn.disabled = !canExpand || state.showAll;
  ...
  function computeTrim(settings) {
    ...
    let hiddenTurns = 0;
    if (enabled && nodes.length > minTurnsBeforeTrim && !state.showAll) {
      const visibleCount = Math.min(nodes.length, keepCount + state.extraVisible);
      hiddenTurns = Math.max(0, nodes.length - visibleCount);
    }

    return { nodes, method, hiddenTurns, totalTurns: nodes.length, hasMain: !!getMain() };
  }
  ...
  function applyTrim(settings) {
    ...
    if (info.hiddenTurns > 0) {
      info.nodes.slice(0, info.hiddenTurns).forEach((el) => el.setAttribute(HIDDEN_ATTR, "true"));
    }
