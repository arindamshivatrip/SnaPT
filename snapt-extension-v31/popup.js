...
function formatDiag(response, pageDiag) {
  ...
  if (pageDiag?.diag) {
    ...
    lines.push(`Recent thinking turns kept: ${d.recentThinkingTurns ?? "n/a"}`);
  ...
  lines.push("v28 keeps thinking/activity for the last 2 live turns, older kept turns stay minimal, and cached history is rendered turn-by-turn.");
  return lines.join("\n");
}
